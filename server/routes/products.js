...
WHERE p.product_id = ?
    ...
    if (product.length === 0) {
      return res
        .status(404)
        .json({ error: `Товар із ID ${product_id} не знайдено` });
    }

    res.json(product[0]);
  } catch (error) {
    console.error("Помилка отримання товару:", error.message);
...
router.post("/", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Товар добавлен", productId: result.insertId });
  } catch (error) {
...
router.put("/:product_id", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    if (existingProduct.length === 0) {
      return res
        .status(404)
        .json({ error: `Товар с ID ${product_id} не найден` });
    }
